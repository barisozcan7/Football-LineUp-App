...
extension UIColor {
    convenience init(hexString8: String) {
        let hex = hexString8.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int = UInt64()
...
extension FourFourTwoViewController : PlayerDataSourceDelegate {
...
extension FourFourTwoViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "442Cell", for: indexPath) as! FourFourTwoTableViewCell
        if let one = one{
...
class FourFourTwoViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let selected = selectedOwnOrBest{
            if(selected == 1){
                ...
                playerDataSource.loadListOfPlayersFor442()
            }
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "showPlayerDetails"){
            ...
            destination.selectedFormation = "SelectedFourFourTwo"
        }
    }
