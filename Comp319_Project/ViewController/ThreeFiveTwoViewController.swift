...
extension UIColor {
    convenience init(hexString7: String) {
        let hex = hexString7.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int = UInt64()
...
extension ThreeFiveTwoViewController : PlayerDataSourceDelegate {
...
extension ThreeFiveTwoViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "352Cell", for: indexPath) as! ThreeFiveTwoTableViewCell
        if let one = one{
...
class ThreeFiveTwoViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let selected = selectedOwnOrBest{
            if(selected == 1){
                ...
                playerDataSource.loadListOfPlayersFor352()
            }
        }
        setBackground()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if(segue.identifier == "showPlayerDetails"){
            ...
            destination.selectedFormation = "SelectedThreeFiveTwo"
        }
    }
